...
class FlockingCloudDataset(data.Dataset):
    ...
    def __init__(self, d, npoints, root, train = False, scale_x = None, scale_y = None):
        ...
        self.x_data = torch.from_numpy(xy[100:,:23000]) # first (measures,velocities)
        self.y_data = torch.from_numpy(xy[100:,23000:]) # true final measures
...
if __name__ == '__main__':
    ...
    batchsize, lr, seed, maxiter, retrain = [16, 1e-2, 48, 200, False]

    RESULT_DIR = os.path.join('data', '.'.join(__file__.split('.')[:-1]))
    os.makedirs(RESULT_DIR, exist_ok=True)
    token = 'trained_model_path'.format(lr, batchsize)
    print('Params: {}'.format(token))
    test_path = os.path.join(RESULT_DIR, token)
    os.makedirs(test_path, exist_ok=True)

    writer = SummaryWriter(os.path.join(test_path,'runs/training_loss'))
    writer_dist = SummaryWriter(os.path.join(test_path,'runs/test_dists'))
    ...
    train_class = FlockingCloudDataset(d=args.d, npoints=args.npoints, root= train_data_path)


    train_loader = data.DataLoader(dataset= train_class,
    ...
    if args.retrain:
        print('retrain!!')
        model_path = 'data/trained_model_path'
        model.load_state_dict(torch.load(os.path.join(model_path, 'model_params_999.pkl')))



    for epoch in range(args.epochs):

        mean_loss = train_v2(epoch, model, args, train_loader, device_ids, optimizer, scheduler)
        writer.add_scalar('train_loss', mean_loss, global_step=epoch)

        torch.save(model.state_dict(), os.path.join(test_path, 'model_params_{}.pkl'.format(epoch)))
